...
class Downloader:
	...
	async def __down(self, queue):
		...
		while not queue.empty():
			...
			async with session.get(url, headers=headers, proxy=self.proxy_path, timeout=20) as resp:
	...
	async def __get_length(self, session, path, url):
		"""创建占位文件，返回文件长度"""
		async with session.get(url, headers=self.__headers, proxy=self.proxy_path, timeout=20) as resp:
			# 获取视频长度
			try:
